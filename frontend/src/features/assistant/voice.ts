...
export class VoiceController {
  ...
  private receivedAudioFrames = false;
  ...
  async start(locale: string): Promise<void> {
    ...
    try {
      ...
      this.captureNode.port.onmessage = (event: MessageEvent<Float32Array>) => {
        ...
        const now = Date.now();
        const hasSpeech = samples.some((sample) => Math.abs(sample) > 0.015);
        this.receivedAudioFrames = true;
      ...
      this.captureNode.onprocessorerror = () => {
        this.fail("Microphone processing stopped. Reopen voice mode or continue by typing.");
      };
      source.connect(this.captureNode);

      this.lastActivityAt = Date.now();
      this.receivedAudioFrames = false;
      this.detectedSpeech = false;
      this.captureTimer = window.setTimeout(() => {
        if (!this.receivedAudioFrames) {
          this.fail("The microphone is not delivering audio. Check the selected input device and try again.");
        }
      }, 4_000);
      this.speechTimer = window.setTimeout(() => {
        if (!this.detectedSpeech && this.isActive) {
          this.setState("listening", "No speech detected — check your microphone input.");
        }
      }, 10_000);
  ...
  async stop(status = "Voice off"): Promise<void> {
    if (!this.isActive && !this.session && !this.mediaStream) {
      return;
    }
    this.stopping = true;
    try {
